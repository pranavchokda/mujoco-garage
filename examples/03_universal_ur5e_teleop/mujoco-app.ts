...
// Keyboard teleop: each entry maps a key to a joint index and direction (+1 or -1)
const KEY_BINDINGS: Record<string, { joint: number; dir: number }> = {
  'q': { joint: 0, dir:  1 },
  'a': { joint: 0, dir: -1 },
  'w': { joint: 1, dir:  1 },
  's': { joint: 1, dir: -1 },
...
// Radians applied per simulation frame while key is held
const TELEOP_STEP = 0.008;
const CTRL_CLAMP  = 3.14;
...
export class MujocoApp {
  ...
  private valSpanEls: HTMLSpanElement[] = [];
  ...
  private buildUI() {
    ...
    // Key hints per joint index (positive key / negative key)
    const keyHints = ['Q/A', 'W/S', 'E/D', 'R/F', 'T/G', 'Y/H'];
    ...
    for (let i = 0; i < model.nu; i++) {
      ...
      if (i < keyHints.length) {
        const hint = document.createElement('span');
        hint.className = 'key-hint';
        hint.textContent = keyHints[i];
    ...
    resetBtn.addEventListener('click', () => {
      ...
      this.mujoco.mj_forward(this.model, this.data);
      this.sliderEls.forEach(el => { el.value = '0'; });
      this.valSpanEls.forEach(el => { el.textContent = '0.00'; });
    });
    panel.appendChild(resetBtn);

    document.body.appendChild(panel);

    // Keyboard help overlay (bottom-left)
    const helpEl = document.createElement('div');
    helpEl.id = 'keyboard-help';
    helpEl.innerHTML = `
      <b>Keyboard Teleop</b>
      <table>
        <tr><td class="kh-keys">Q / A</td><td>shoulder_pan  ±</td></tr>
        <tr><td class="kh-keys">W / S</td><td>shoulder_lift ±</td></tr>
        <tr><td class="kh-keys">E / D</td><td>elbow         ±</td></tr>
        <tr><td class="kh-keys">R / F</td><td>wrist_1       ±</td></tr>
        <tr><td class="kh-keys">T / G</td><td>wrist_2       ±</td></tr>
        <tr><td class="kh-keys">Y / H</td><td>wrist_3       ±</td></tr>
      </table>
    `;
  ...
  private applyKeyboardControl() {
    for (const key of this.keysHeld) {
      const binding = KEY_BINDINGS[key];
      if (!binding) continue;
      const { joint, dir } = binding;
      if (joint >= this.model.nu) continue;

      const next = Math.max(-CTRL_CLAMP, Math.min(CTRL_CLAMP, this.data.ctrl[joint] + dir * TELEOP_STEP));
      this.data.ctrl[joint] = next;

      // Keep slider and value display in sync
      if (this.sliderEls[joint]) this.sliderEls[joint].value = String(next);
      if (this.valSpanEls[joint]) this.valSpanEls[joint].textContent = next.toFixed(2);
    }
  }
